# LQ-360 — Read-only Runtime Cleanup Chained Continuation Claim Inspector

## Ergebnis

LQ-360 installiert `liquent-disposable-postgres-cleanup-chain-reconcile` als
strikt read-only Inspector für offene LQ-358-Claims.

Er klassifiziert ausschließlich den aktuellen Cleanup-Präfix und verändert
weder Claim, Evidence noch Dockerressource.

## Neue Autorisierung

Die owner-only Chained-Reconciliation-Autorisierung bindet geschlossen:

- Chained-Reconciliation- und Chained-Continuation-ID;
- vollständige Recontinuation-, Continuation-, Cleanup- und Run-Kette;
- Source, Image, Compose sowie sämtliche Evidence- und Autorisierungshashes;
- SHA-256 der vollständigen LQ-358-Autorisierung;
- SHA-256 der exakten LQ-349- und LQ-355-Finalization-Evidence;
- historischen und effektiven Startpräfix;
- Scope `runtime_only`;
- Operation `inspect_disposable_postgres_cleanup_chained_continuation`;
- getrennte Executor- und Autorisiereridentitäten;
- ein aktuelles UTC-Fenster von höchstens einer Stunde.

Unbekannte Felder, doppelte Schlüssel, stale Zeit oder Hashabweichung bleiben
detailfrei unavailable.

## Historische Bindung

LQ-358 und LQ-355 werden an ihren ursprünglichen Fenstermittelpunkten erneut
validiert.

Die neue Autorisierung verlängert weder Mutation noch frühere Reconciliation.

IDs, Hashes, beide Startpräfixe, Run und Projektname müssen exakt dieselbe
Kette beschreiben.

Die LQ-355-Evidence muss weiterhin einen der beiden nichtterminalen Ausgänge
enthalten und denselben effektiven Präfix ableiten.

## Claim-Gates

Der ursprüngliche LQ-339-Cleanup-Claim wird kanonisch geprüft und muss offen
bleiben.

Seine Abwesenheit ergibt `conflict`; malformed Bindung bleibt unavailable.

Alter LQ-345-Continuation-Claim und LQ-351-Recontinuation-Claim müssen exakt
fehlen.

Ein vorhandener historischer Claim wird validiert, aber technisch abgelehnt
und niemals freigegeben.

Der aktuelle Claim wird nur aus dem SHA-256 der Chained-Continuation-ID
abgeleitet.

## Evidence-first

Exakt gebundene LQ-358-Evidence wird vor LQ-341 geprüft.

Ist sie vorhanden, lautet der Ausgang
`chained_continuation_evidence_present`.

Ein gleichzeitig offener aktueller Claim wird nicht entfernt.

Fehlen Evidence und Claim gemeinsam, lautet der Ausgang `not_found` ohne
Dockerzugriff.

Evidence ohne Claim bleibt ebenfalls
`chained_continuation_evidence_present`.

Beschädigte oder widersprüchliche Evidence ist keine neutrale Abwesenheit.

## Kanonischer Claim

Der Claim muss owner-only, regulär, einfach verlinkt und vollständig gegen die
LQ-358-Bindung validierbar sein.

Er bindet historische Autorität, beide Finalization-Evidencehashes, beide
Startpräfixe, Restbudget, Ressourcen, Identitäten und UTC-Startzeit.

Alter und Dateiname beweisen keinen Fortschritt.

Der Inspector repariert oder ersetzt keinen Claim.

## Frische LQ-341-Inspection

Nur bei offenem Cleanup- und Chained-Claim ohne Evidence wird LQ-341 frisch
mit historischer Autorisierung ausgeführt.

Die Ausgabe muss exakt Schema-Version, Operation
`disposable_postgres_runtime_cleanup_reconciliation` und einen geschlossenen
Ausgang enthalten.

Caller-gelieferte oder gespeicherte Beobachtungen werden nicht akzeptiert.

LQ-341 bestätigt Ressourcenordnung und unverändert rungebundenes Datenvolume
read-only.

## Geschlossene Präfixmatrix

Ab effektivem `container_removed` sind zulässig:

- derselbe Zustand als `chained_continuation_not_started`;
- `application_network_removed`;
- `runtime_removed_evidence_missing`.

Ab effektivem `application_network_removed` sind zulässig:

- derselbe Zustand als `chained_continuation_not_started`;
- `runtime_removed_evidence_missing`.

Jeder frühere oder außerhalb liegende Zustand ergibt `conflict`.

Dies umfasst `container_stopped`, `runtime_intact`, `final_evidence_present`,
unmögliche Ressourcenbilder und Volumeabweichung.

Kein Ausgang erteilt Fortsetzungs- oder Finalisierungsrecht.

## Strikte Read-only-Grenze

LQ-360 besitzt keinen Write-, Release-, Stop-, Start-, Remove-, Disconnect-,
Down-, Kill-, Prune- oder Volume-Mutationspfad.

Auch bei vollständiger Runtimeentfernung bleiben Cleanup- und aktueller Claim
unverändert.

Docker-Events, Logs, Historie, SQL und Volumeinhalte werden nicht gelesen.

Technische Nichtverfügbarkeit endet ohne Ergebnisobjekt oder private Details.

## Ausgabe

Die CLI liefert nur Schema-Version, Operation
`disposable_postgres_cleanup_chained_continuation_reconciliation` und:

- `not_found` oder `chained_continuation_evidence_present`;
- `chained_continuation_not_started`;
- `application_network_removed`;
- `runtime_removed_evidence_missing`;
- `conflict`.

Private IDs, Hashes, Pfade und Ressourcen verlassen die Grenze nicht.

## Tests

Fake-basierte Tests decken die vollständige Präfixmatrix für beide effektiven
Startzustände ab.

Claims werden vor und nach jeder Entscheidung bytegenau verglichen.

Weitere Tests beweisen Evidence-first und `not_found` ohne Aufruf des
LQ-341-Inspectors.

## Bundle und Nichtziele

Entry Point und Operatormodul erhöhen die Gates auf 45 Entry Points und 49
Operatormodule. Migrationen bleiben 27 mit Head `20260819_0027`.

LQ-360 implementiert keine Claimfreigabe, Evidencepersistenz, weitere
Continuation, Cleanup-Finalisierung oder Volume-Löschung.

Es gibt keine Schema-, Tabellen-, SQL-, Migration-, Port-, Domainmodell-,
Compose- oder Production-Wiring-Änderung.

## Nächster Slice

LQ-361 sollte die evidence-first Finalisierung eines reconcilierten
Chained-Continuation-Claims getrennt definieren.

Weitere Fortsetzung und jede Volumenlöschung bleiben separate Entscheidungen.
