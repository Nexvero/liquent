...
## Zweck
LQ-368 definiert die kontrollierte Finalisierung eines durch LQ-367
...
exakten aktuellen Generation-Claims und implementiert keinen Command.
...
- SHA-256 der LQ-365- und LQ-367-Autorisierung;
...
Der Finalizer validiert Root-, LQ-362-, LQ-365- und LQ-367-Kette erneut.
...
Der erste Finalizer akzeptiert ausschließlich Generation eins mit direktem
LQ-362-Vorgänger.
LQ-362-Finalization-Evidence muss weiterhin owner-only, kanonisch, bytegenau
...
LQ-368 definiert bereits das wiederverwendbare Evidenceformat, öffnet aber
...
Nur der aktuelle Generation-Claim liegt in der Freigabegrenze.
...
Exakt gebundene Finalization-Evidence wird vor LQ-367 geprüft und steuert den
idempotenten Retry.
...
## Frische LQ-367-Entscheidung
Ohne Finalization-Evidence muss der Finalizer LQ-367 unmittelbar mit derselben
...
einem geschlossenen LQ-367-Ausgang sein.
LQ-367 darf nur bei offenem Cleanup- und aktuellem Claim ohne Evidence
...
Sie ist der einzige zulässige direkte Vorgängeranker für Generation zwei.

LQ-368 erzeugt keine Generation-Continuation- oder Cleanup-Evidence
nachträglich.
...
Ein vorhandener Claim muss vollständig gegen dieselbe LQ-365-Bindung geprüft
...
Der Retry führt weder LQ-367 noch Docker erneut aus und schreibt keine zweite
Evidence.
...
Generation zwei muss eine neue ID, Generation exakt zwei,
`predecessor_kind=repeatable_generation`, Vorgängergeneration eins und SHA-256
...
LQ-368 implementiert keinen Finalizer, Entry Point, Test, Folgeoperator,
...
LQ-369 sollte den evidence-first Generation-Finalizer samt Fake-basierten
Tests für alle geschlossenen Ausgänge implementieren.

Die Ausführung von Generation zwei bleibt ein separater späterer Slice.
