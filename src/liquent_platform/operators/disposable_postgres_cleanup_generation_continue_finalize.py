"""Evidence-first finalization of a reconciled generation continuation."""
from __future__ import annotations
import argparse,hashlib,json,os,stat,sys
from datetime import UTC,datetime,timedelta
from pathlib import Path
from liquent_platform.operators.disposable_postgres_cleanup_generation_continue import _bind,_file,_lineage
from liquent_platform.operators.disposable_postgres_cleanup_generation_continue_reconcile import COMPARE,_authorization as _reconcile_auth,_historical_generation,reconcile_disposable_postgres_cleanup_generation_continuation
...
FINAL={"generation_continuation_evidence_present":"generation_continuation_evidence_confirmed","generation_continuation_not_started":"generation_continuation_attempt_finalized","application_network_removed":"later_prefix_finalized","runtime_removed_evidence_missing":"runtime_removal_ready_for_cleanup_finalization"}
HIST=("generation_reconciliation_id",*COMPARE,"generation_continuation_authorization_sha256")
KEYS={"schema_version","generation_finalization_id",*HIST,"generation_reconciliation_authorization_sha256","operation","scope","executor_id","authorizer_id","valid_from","valid_until"}
class DisposablePostgresCleanupGenerationContinueFinalizeUnavailable(Exception):
    code="disposable_postgres_cleanup_generation_continue_finalize_unavailable"
    def __init__(self):super().__init__(self.code)
class _Parser(argparse.ArgumentParser):
    def error(self,_):raise DisposablePostgresCleanupGenerationContinueFinalizeUnavailable
def _authorization(path,clock):
    try:
        v=json.loads(_private_file(path,32768),object_pairs_hook=_pairs)
        if type(v) is not dict or set(v)!=KEYS or v["schema_version"]!=1 or v["phase"]!="disposable_postgres" or v["scope"]!="runtime_only" or v["operation"]!="finalize_disposable_postgres_cleanup_generation_continuation" or type(v["generation"]) is not int or v["generation"]<1 or type(v["predecessor_generation"]) is not int or v["predecessor_kind"] not in {"lq362","repeatable_generation"} or (v["generation"]==1)!=(v["predecessor_kind"]=="lq362") or v["predecessor_generation"]!=v["generation"]-1 or v["resume_from"] not in {"container_removed","application_network_removed"} or v["predecessor_resume_from"] not in {"container_removed","application_network_removed"}:raise ValueError
        for k in ("generation_finalization_id","generation_reconciliation_id","generation_continuation_id","chained_finalization_id","chained_reconciliation_id","chained_continuation_id","recontinuation_finalization_id","recontinuation_reconciliation_id","recontinuation_id","continuation_finalization_id","continuation_reconciliation_id","cleanup_continuation_id","cleanup_reconciliation_id","cleanup_id","run_id","reconciliation_id","claim_reconciliation_id","disposition_id","executor_id","authorizer_id"):
        ...
        return v
    except Exception:raise DisposablePostgresCleanupGenerationContinueFinalizeUnavailable from None
...
def _historical_finalization(path):
    try:v=json.loads(_private_file(path,32768),object_pairs_hook=_pairs);a,b=_timestamp(v["valid_from"]),_timestamp(v["valid_until"]);return _authorization(path,lambda:a+(b-a)/2)
    except Exception:raise DisposablePostgresCleanupGenerationContinueFinalizeUnavailable from None
def _binding5(v,path):return {"schema_version":1,**{k:v[k] for k in ("generation_finalization_id",*HIST,"scope","executor_id","authorizer_id")},"finalization_authorization_sha256":hashlib.sha256(_private_file(path,32768)).hexdigest()}
...
def _result(outcome):return (json.dumps({"operation":"disposable_postgres_cleanup_generation_continuation_finalization","outcome":outcome,"schema_version":1},sort_keys=True,separators=(",",":"))+"\n").encode()
def finalize_disposable_postgres_cleanup_generation_continuation(*,docker_executable,authorization_file,reconciliation_file,claim_reconciliation_file,disposition_file,cleanup_file,cleanup_reconciliation_file,cleanup_continuation_file,continuation_reconciliation_file,continuation_finalization_file,recontinuation_file,recontinuation_reconciliation_file,recontinuation_finalization_file,chained_continuation_file,chained_finalization_file,generation_continuation_file,generation_reconciliation_file,generation_finalization_file,staging_evidence_file,compose_file,runtime_environment_file,image_environment_file,project_name,evidence_directory,predecessor_generation_continuation_file=None,predecessor_generation_finalization_file=None,generation_lineage_continuation_files=None,generation_lineage_finalization_files=None,processes=None,clock=lambda:datetime.now(UTC)):
    fd=None
    try:
        original=_historical(authorization_file);cleanup=_historical_cleanup(cleanup_file);old=json.loads(_private_file(cleanup_continuation_file,32768),object_pairs_hook=_pairs);recont=_historical_recontinuation(recontinuation_file);chain=json.loads(_private_file(chained_continuation_file,32768),object_pairs_hook=_pairs);generation=_historical_generation(generation_continuation_file);recon=_historical_reconciliation(generation_reconciliation_file);current=_authorization(generation_finalization_file,clock)
        if current["generation_reconciliation_authorization_sha256"]!=hashlib.sha256(_private_file(generation_reconciliation_file,32768)).hexdigest() or any(current[k]!=recon[k] for k in HIST) or original.run_id!=current["run_id"] or project_name!=f"liquent-{original.run_id}":raise ValueError
        if current["generation"]==1:
            ...
            predecessor=_historical_generation(predecessor_generation_continuation_file);last=_historical_finalization(predecessor_generation_finalization_file)
            if predecessor["generation"]!=1 or last["generation"]!=1 or current["predecessor_resume_from"]!=predecessor["resume_from"] or current["predecessor_finalization_authorization_sha256"]!=hashlib.sha256(_private_file(predecessor_generation_finalization_file,32768)).hexdigest():raise ValueError
            lb=_binding5(last,predecessor_generation_finalization_file);ls=hashlib.sha256(last["generation_finalization_id"].encode()).hexdigest();lp=evidence_directory/f"postgres-cleanup-generation-continuation-finalization-{ls}.json";out=_existing(lp,lb)
            if current["predecessor_finalization_evidence_sha256"]!=hashlib.sha256(_private_file(lp,32768)).hexdigest() or out not in {"generation_continuation_attempt_finalized","later_prefix_finalized"}:raise ValueError
            ...
            _lineage(current,generation_lineage_continuation_files,generation_lineage_finalization_files,chained_finalization_file,evidence_directory,project_name)
        ...
        bind=_bind(generation,generation_continuation_file,project_name);stem=hashlib.sha256(generation["generation_continuation_id"].encode()).hexdigest();claim=evidence_directory/f".postgres-cleanup-generation-continuation-{stem}.claim";fb=_binding5(current,generation_finalization_file);fs=hashlib.sha256(current["generation_finalization_id"].encode()).hexdigest();final=evidence_directory/f"postgres-cleanup-generation-continuation-finalization-{fs}.json";fd=_evidence_root(evidence_directory);existing=_existing(final,fb)
        if existing:
            if claim.exists() and not _file(claim,bind):raise ValueError
            if claim.exists():os.unlink(claim);os.fsync(fd)
            return _result(existing)
        a,b=_timestamp(recon["valid_from"]),_timestamp(recon["valid_until"]);raw=reconcile_disposable_postgres_cleanup_generation_continuation(docker_executable=docker_executable,authorization_file=authorization_file,reconciliation_file=reconciliation_file,claim_reconciliation_file=claim_reconciliation_file,disposition_file=disposition_file,cleanup_file=cleanup_file,cleanup_reconciliation_file=cleanup_reconciliation_file,cleanup_continuation_file=cleanup_continuation_file,continuation_reconciliation_file=continuation_reconciliation_file,continuation_finalization_file=continuation_finalization_file,recontinuation_file=recontinuation_file,recontinuation_reconciliation_file=recontinuation_reconciliation_file,recontinuation_finalization_file=recontinuation_finalization_file,chained_continuation_file=chained_continuation_file,chained_finalization_file=chained_finalization_file,generation_continuation_file=generation_continuation_file,generation_reconciliation_file=generation_reconciliation_file,staging_evidence_file=staging_evidence_file,compose_file=compose_file,runtime_environment_file=runtime_environment_file,image_environment_file=image_environment_file,project_name=project_name,evidence_directory=evidence_directory,predecessor_generation_continuation_file=predecessor_generation_continuation_file,predecessor_generation_finalization_file=predecessor_generation_finalization_file,generation_lineage_continuation_files=generation_lineage_continuation_files,generation_lineage_finalization_files=generation_lineage_finalization_files,processes=processes,clock=lambda:a+(b-a)/2);obs=json.loads(raw,object_pairs_hook=_pairs)
        if type(obs) is not dict or set(obs)!={"schema_version","operation","outcome"} or obs["schema_version"]!=1 or obs["operation"]!="disposable_postgres_cleanup_generation_continuation_reconciliation":raise ValueError
        state=obs["outcome"]
...
def main(argv=None):
    ...
    try:v=vars(p.parse_args(argv));v["runtime_environment_file"]=v.pop("runtime_env_file");v["image_environment_file"]=v.pop("image_env_file");sys.stdout.buffer.write(finalize_disposable_postgres_cleanup_generation_continuation(**v));return 0
