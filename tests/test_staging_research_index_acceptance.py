...
@pytest.mark.parametrize("field", ["check", "outcome"])
def test_non_enum_observation_values_reject(field: str) -> None:
    observations = list(_observations())
    first = observations[0]
    observations[0] = StagingResearchIndexObservation(
        cast(
            StagingResearchIndexCheck,
            first.check.value if field == "check" else first.check,
        ),
        cast(
            StagingResearchIndexCheckOutcome,
            first.outcome.value if field == "outcome" else first.outcome,
        ),
    )
