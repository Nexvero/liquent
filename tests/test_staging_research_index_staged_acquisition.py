...
def test_untyped_session_slot_fails_before_acquisition() -> None:
    acquisition = Acquisition()
    stage = StagingResearchIndexAcquisitionStage.AFTER_REVOCATION
    sessions = {
        StagingResearchIndexCredentialSlot.REVOCATION_FIXTURE_READER.value:
            SESSIONS[StagingResearchIndexCredentialSlot.REVOCATION_FIXTURE_READER]
    }
    with pytest.raises(ValueError):
        acquire_staging_research_index_stage(RUN, stage, acquisition, sessions)
    assert acquisition.calls == []
