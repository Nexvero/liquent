...
@pytest.mark.parametrize("field", ("check", "phase", "outcome"))
def test_reducer_rejects_untyped_classification_fields(field) -> None:
    values = list(_complete())
    item = values[0]
    values[0] = StagingResearchIndexResponseClassification(
        "anonymous_closed" if field == "check" else item.check,
        "single" if field == "phase" else item.phase,
        "passed" if field == "outcome" else item.outcome,
    )
    with pytest.raises(ValueError):
        reduce_staging_research_index_classifications(tuple(values))
