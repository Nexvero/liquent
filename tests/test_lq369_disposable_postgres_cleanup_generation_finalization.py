...
from tests.test_lq367_disposable_postgres_cleanup_generation_reconciliation import setup
def prepare(tmp_path:Path):
    values,cleanup,claim,_,_,_=setup(tmp_path,"chained_continuation_not_started");r=json.loads(values["generation_reconciliation_file"].read_text());auth={"schema_version":1,"generation_finalization_id":"generation-finalization-369",**{k:r[k] for k in final.HIST},"generation_reconciliation_authorization_sha256":hashlib.sha256(values["generation_reconciliation_file"].read_bytes()).hexdigest(),"operation":"finalize_disposable_postgres_cleanup_generation_continuation","scope":"runtime_only","executor_id":"generation-finalizer","authorizer_id":"finalization-authorizer","valid_from":"2026-08-20T13:30:00Z","valid_until":"2026-08-20T14:30:00Z"};values["generation_finalization_file"]=_private(tmp_path/"generation-finalization.json",auth);values["clock"]=lambda:NOW;return values,cleanup,claim
...
def test_finalizable_states_write_evidence_then_release_only_current_claim(tmp_path,monkeypatch,state,outcome):
    values,cleanup,claim=prepare(tmp_path);observed(monkeypatch,state);result=final.finalize_disposable_postgres_cleanup_generation_continuation(**values);assert json.loads(result)["outcome"]==outcome;stem=hashlib.sha256(b"generation-finalization-369").hexdigest();record=json.loads((values["evidence_directory"]/f"postgres-cleanup-generation-continuation-finalization-{stem}.json").read_text());assert record["observed_state"]==state and record["outcome"]==outcome and record["generation"]==1 and cleanup.exists() and not claim.exists()
...
def test_neutral_states_do_not_write_or_release(tmp_path,monkeypatch,state,outcome):
    values,cleanup,claim=prepare(tmp_path);observed(monkeypatch,state);assert json.loads(final.finalize_disposable_postgres_cleanup_generation_continuation(**values))["outcome"]==outcome and cleanup.exists() and claim.exists();assert not list(values["evidence_directory"].glob("postgres-cleanup-generation-continuation-finalization-*.json"))
