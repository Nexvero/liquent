...
TARGET = (
    ...
    "staging_research_index_promotion_reconciliation_settings_installation_cli:main"
)


def test_settings_installation_cli_has_one_exact_installed_entry_point() -> None:
    project = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert project.count(f'{COMMAND} = "{TARGET}"') == 1


def test_release_inventory_tracks_command_without_operator_drift() -> None:
    project = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
...
def test_entry_point_targets_existing_detail_free_cli_boundary() -> None:
    module = (
        ROOT
        / "src/liquent_platform/transport"
        / "staging_research_index_promotion_reconciliation_settings_installation_cli.py"
